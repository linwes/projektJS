...
const uppanswer = async (data) =>{
    try {
        const dbocn = await dbPromise;
        await dbocn.run('UPDATE answer SET answerText=(?), questId=(?) WHERE id=(?)', [data.answerText, data.questId, data.id]);
    }
    catch (error) {
        console.log('något gick fel med updatering i databas')
    }
...
const image = async (fileName, id) => {
    ...
    {
        console.log('något gick fel med att spara bild i databasen')
    }
}
module.exports = {
    getUsers: getUsers,
